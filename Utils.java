...
public class Utils {
	...
	public static void hyperReal(ArrayList<FinalEntry> all, int year, float bankroll, float percent) {
		...
		for (FinalEntry i : all) {
			...
			bank=UtilsControls.bankControls(cal,month,i,betSize,succ,alls,bank);
			bank=UtilsControls.bankNotControls(cal,month,i,betSize,succ,alls,bank);
		}
...
//		System.out.println(fixtures);
	...
	public static float evaluatePlayers(ExtendedFixture ef, ArrayList<PlayerFixture> pfs,
		...
		returned=UtilsControls.expectedControl(expected,avgShotsOver,avgShotsUnder,dist);
		returned=UtilsControls.expectedNotControl(expected,avgShotsOver,avgShotsUnder,dist);
		return returned;
			...
			for (int x = 0; x <= xmax; x++) {
				...
				for (int y = 0; y <= ymax; y++) {
					...
					for (int z = 0; z <= zmax; z++) {
						...
						currEval=UtilsControls.newMaxByControl1(newMaxBy,all,currentProfit,currEval);
						
						currEval=UtilsControls.newMaxyByControl2(newMaxBy,all,currentProfit,currEval);
						
						currEval=UtilsControls.newMaxByControl3(newMaxBy,all,currentProfit,currEval);

						UtilsControls.finalControl4(currEval, bestEval, bestProfit, bestWinRatio, bestTH, step,  bestDescription, 
								 x,  y,  z,  w,  bestx,  besty,  bestz,  bestw,  currentProfit,  currentWinRate, currentTH)
